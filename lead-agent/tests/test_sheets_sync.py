...
def test_sync_retries_on_read_timeout(monkeypatch):
    monkeypatch.setenv("GOOGLE_SHEETS_WEBHOOK_URL", "https://script.google.com/macros/s/test/exec")

    post_response = MagicMock()
    post_response.status_code = 200
    post_response.text = "ok"

    mock_client = MagicMock()
    mock_client.post.side_effect = [httpx.ReadTimeout("slow"), post_response]
    mock_client.__enter__ = MagicMock(return_value=mock_client)
    mock_client.__exit__ = MagicMock(return_value=False)

    lead = {"id": 3, "name": "Retry", "phone": "91", "source": "x", "status": "new", "owner_phone": "91"}

    with patch("app.integrations.sheets.httpx.Client", return_value=mock_client):
        with patch("app.integrations.sheets.time.sleep"):
            assert sync_lead_to_sheet(lead) is True
